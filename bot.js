...
const COMMANDS = {
    ...
    adduser: async (msg, args) => {
        ...
        if (data.ok)
            return msg.reply({ embeds: [embed("✅ User Added", `**${user}** has been whitelisted.\nPassword: \`${pass}\``, GREEN)] });
    ...
    setpass: async (msg, args) => {
        ...
        if (data.ok)
            return msg.reply({ embeds: [embed("🔑 Password Updated", `Password for **${user}** has been changed to \`${pass}\``, BLUE)] });
        if (data.error === "not_found")
            return msg.reply({ embeds: [embed("Not Found", `**${user}** is not whitelisted.`, RED)] });
        return msg.reply({ embeds: [embed("Error", data.error ?? "Unknown error", RED)] });
    },

    // !deactivate <user>  — disables login without deleting
    ...
    activate: async (msg, args) => {
        ...
        if (data.ok)
            return msg.reply({ embeds: [embed("▶️ Activated", `**${user}** has been reactivated and can log in again.`, GREEN)] });
        if (data.error === "not_found")
            return msg.reply({ embeds: [embed("Not Found", `**${user}** is not whitelisted.`, RED)] });
    ...
    checkuser: async (msg, args) => {
        ...
        const data = await get("/list");
        const users = data.users ?? [];
        const found = users.find(u => u.name.toLowerCase() === user.toLowerCase());
        if (found) {
            const status = found.inactive ? "⏸️ Deactivated" : "✅ Active";
            return msg.reply({ embeds: [embed("Found", `**${user}** is whitelisted.\nStatus: ${status}`, found.inactive ? YELLOW : GREEN)] });
        }
        return msg.reply({ embeds: [embed("❌ Not Found", `**${user}** is NOT whitelisted.`, RED)] });
    },
    ...
    help: async (msg) => {
        return msg.reply({ embeds: [embed("📖 Commands", [
            ...
            "`!deactivate <user>` — block a user without deleting them",
            "`!listusers` — show all users + their status",
